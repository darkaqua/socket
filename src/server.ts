...
export const getServerSocket =
  ...
  ({
    ...
    ping,
    pingEvery = 1000,
    next = () => new Response(null, { status: 501 }),
    onListen = undefined,
  }: ServerProps): ServerSocket => {
      ...
      async (request: Request, connInfo) => {
        ...
        const client: ServerClient = {
          ...
          remove: (event: string, id: number) =>
            (clientEvents[event] = clientEvents[event].filter(
              (client, index) => (index === id ? undefined : client),
            )),
          rooms: [],
          getRooms: () =>
            clientList[clientId].rooms.map((name) => roomList[name]),
          addRoom: (name: string) => {
            if (clientList[clientId].rooms.includes(name)) {
              return console.warn(
                `Client ${clientId} is already in room ${name}!`,
              );
            }
